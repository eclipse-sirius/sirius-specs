...
The note attachments link a purely graphical element (a `Note`, a `Representation Link`, a `Text` or another `Note Attachment`) to another object. This object is most of the time a Sirius graphical element, which correspond to a semantic element. But it can also be another purely graphical element.
